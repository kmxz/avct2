...
export const send = function (api: keyof typeof actions, opt_params: { [key: string]: any } = {}): Promise<any> {
    ...
    if (config.method === 'post') {
        if (('params' in config) && config.params.length) {
    ...
    return throttle.add(function (resolve, reject) {
        ...
        xhr.onload = function () {
            if (xhr.status === 200) {
                ...
                const err = xhr.getResponseHeader("X-Error");
                countError();
                if (!(toReload || muted)) {
...
export const FATAL = (): void => {
    if (!toReload) {
        window.alert("You just ignored a fatal error. The program will fail to function if you do not reload.");
    }
};

export const loadImage = (response, opt_onload, opt_onerror): HTMLImageElement => {
    var image = new Image();
    var url = window.URL.createObjectURL(response);
    var onload = opt_onload || (() => void 0);
    var onerror = opt_onerror || (() => void 0);
