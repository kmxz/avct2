...
export const loadImgToCover = (img: HTMLImageElement, ctx: CanvasRenderingContext2D, noAutoScale?: boolean): void => {
...
export const loadImgUrlToCover = (url: string, ctx: CanvasRenderingContext2D, noAutoScale?: boolean): Promise<void> => new Promise((res, rej) => {
    const img = new Image();
    img.onload = () => { loadImgToCover(img, ctx, noAutoScale); res(); }
    img.onerror = rej;
    img.src = url;
});